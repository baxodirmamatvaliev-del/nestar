...
export class BoardArticleService {
    ...
    public async createBoardArticle(memberId: Types.ObjectId, input: BoardArticleInput): Promise<BoardArticle>{
        input.memberId = memberId //input.memberId sini  memberId tenglayapmz
        try{
            const result = await this.boardArticleModel.create(input) // yangi dokument hosil qilyapmz DB da
    ...
    public async getBoardArticle( memberId: Types.ObjectId, articleId: Types.ObjectId): Promise<BoardArticle> {
      ...
      if(memberId) { // member Auth bolgan member bolsa article view si 1 ga oshadi
    ...
    public async updateBoardArticle(memberId:Types.ObjectId, input: BoardArticleUpdate): Promise<BoardArticle>{
         ...
         .findOneAndUpdate({ _id: _id ,memberId: memberId ,  // _id: _id article ID si ozgartirmpqchi bolgan article muurajatchini article bolishi kk
            articleStatus: BoardArticleStatus.ACTIVE}, input,
            {
                new: true
            })
            .exec()
...
public async getAllBoardArticlesByAdmin( input: AllBoardArticlesInquiry,): Promise<BoardArticles> {
  ...
  if (articleStatus) { match.articleStatus = articleStatus; } // ADMIN talab etsa match ga qoshib boyityapmz 
  if (articleCategory) { match.articleCategory = articleCategory;}
...
public async removeBoardArticleByAdmin(articleId: Types.ObjectId): Promise<BoardArticle> {
  const search: T = { _id: articleId, articleStatus: BoardArticleStatus.DELETE }; // Faqat statusi DELETE bolgan proprtyni ochira oladi 
