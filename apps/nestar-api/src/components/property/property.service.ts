...
export class PropertyService {
    ...
    public async getProperty(memberId: Types.ObjectId, propertyId: Types.ObjectId): Promise<Property> {
       const search: T = { _id: propertyId, // <-- biz  kormoqchi bolgan property 
        propertyStatus: PropertyStatus.ACTIVE, }; // <-- biz  kormoqchi bolgan property status

    const targetProperty: Property | null = await this.propertyModel.findOne(search).lean().exec();
     if (!targetProperty) throw new InternalServerErrorException(Message.NO_DATA_FOUND);

 if (memberId) { // Auth bolgan member 
  const viewInput = { memberId: memberId,
     viewRefId: propertyId, viewGroup: ViewGroup.PROPERTY }; //  member korgan PROPERTY
 ...
 return targetProperty;
  ...
  public async updateProperty(memberId: Types.ObjectId, input: PropertyUpdate): Promise<Property>{
    let {propertyStatus, soldAt, deletedAt } = input;
     ...
     const search: T ={ 
        _id: input._id,
        memberId: memberId, // <-- AGENT daxildor uy bolishi kk 
     ...
     if(propertyStatus === PropertyStatus.SOLD) soldAt = moment().toDate()
        ...
        if(!result) throw new InternalServerErrorException(Message.UPDATE_FAILED)
            ...
            if(soldAt || deletedAt) { // Sold & delete bogandan song uni property sni -1 kamaytiryapmz 
                await this.memberService.memberStatsEditor({
                 _id: memberId,
...
public async getProperties(memberId: Types.ObjectId, input: PropertiesInquiry): Promise<Properties> {
  ...
  const sort: T = { [input?.sort ?? 'createdAt']: input?.direction ?? Direction.DESC }; // client hech narsa searsh qilmasa yangisindan boshlab eskisigacha chiqaradi 
...
public async getAgentProperties(memberId: Types.ObjectId, input: AgentPropertiesInquiry): Promise<Properties> {
  ...
  const match: T = {
    memberId: memberId,
    propertyStatus: propertyStatus ?? { $ne: PropertyStatus.DELETE }, // Delete dan tashqari bolgani
  };
 ...
 public async likeTargetProperty(memberId: Types.ObjectId, likeRefId: Types.ObjectId ): Promise<Property>{
    const target: Property | null = await this.propertyModel.findOne({
      _id: likeRefId, 
      propertyStatus: PropertyStatus.ACTIVE
    }).exec() // like hosil qluvchi member mavjudmi?
